...
class Analyzer(object) :
   #TODO: maybe initialize the analyzer with the url so the read, lower, encode cycle does not have to be done for every input
   xpath = "//*[contains(normalize-space(.), '{0}') and not(.//*[contains(normalize-space(.), '{0}')])]/*"

   @classmethod
   def search_path(self, url, input_string) :
       html = etree.HTML(urllib.request.urlopen(url).read().decode('utf-8').lower())
       #elem_tree = lh.parse(url)
       elem_tree = etree.ElementTree(html)
       node = elem_tree.xpath(self.xpath.format(input_string.lower()))[0]
